    ...
    chunk_ldv1:
        ...
        seq:
            ...
            - id: particle_bone_lod
              ...
              repeat-expr: 4
              doc: |
                Defines `_lod%02d.skin` files. As an example, on `pandarenfemale.m2`,
                lod_count == 4. SFID has 7 files, the first 4 are ordinary `.skin`
                files, the last 3 are `_lod%02d.skin` files. Enumeration starts
                from 1, and the last file in SFID is `pandarenfemale_lod03.skin`.

                LodData.particleBoneLod works this way: Each model has current lod which is `[0..3]`.
